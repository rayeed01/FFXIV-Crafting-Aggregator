...
- `GET /api/v1/worlds`, `GET /api/v1/data-centers` — open to everyone
...
| Method | Path | Body | Description |
|---|---|---|---|
| GET | `/users/me` | — | Current user profile, including `role` and `createdAt` |
| PATCH | `/users/me/defaults` | `{ defaultDataCenter, defaultWorld }` | Update default world/DC |

### Worlds (`/worlds`, `/data-centers`) — public

Unauthenticated on purpose: registration requires a default world and data center, so the
selector must be populated before the caller has a token.

| Method | Path | Description |
|---|---|---|
| GET | `/worlds` | All synced worlds with their data center and region |
| GET | `/data-centers` | All synced data centers |

Both return **503** (`GameServerDataNotSyncedException`) until `POST /admin/sync/worlds` has run.
